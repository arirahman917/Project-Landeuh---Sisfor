        ...
        <table class="w-full text-sm min-w-[1150px]">
            <thead>
                <tr class="border-b-2 border-amber-300/60">
                    ...
                    <th class="px-3 py-3.5 text-left font-bold text-stone-700 text-xs uppercase tracking-wider">Tanggal Dipesan</th>
                    <th class="px-3 py-3.5 text-center font-bold text-stone-700 text-xs uppercase tracking-wider">Malam</th>
                    <th class="px-3 py-3.5 text-left font-bold text-stone-700 text-xs uppercase tracking-wider w-[260px] sm:w-[300px]">Check-in / Check-out</th>
                    <th class="px-3 py-3.5 text-left font-bold text-stone-700 text-xs uppercase tracking-wider">
                        Tambahan Orang
                        <button onclick="document.getElementById('modalInfoOrang')?.classList.remove('hidden')" class="ml-1 text-stone-400 hover:text-stone-600 transition" title="Info Tambahan Orang">
                            <iconify-icon icon="lucide:info" class="text-xs"></iconify-icon>
                        </button>
                    </th>
                    <th class="px-3 py-3.5 text-right font-bold text-stone-700 text-xs uppercase tracking-wider w-[100px]">Total Bayar</th>
...
{{-- Modal Info Orang --}}
<div id="modalInfoOrang" class="fixed inset-0 z-[10000] hidden bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-stone-100 flex items-center justify-between">
            <h3 class="font-bold text-stone-800">Info Tambahan Orang</h3>
            <button onclick="document.getElementById('modalInfoOrang').classList.add('hidden')" class="text-stone-400 hover:text-stone-600">
                <iconify-icon icon="lucide:x" class="text-xl"></iconify-icon>
            </button>
        </div>
        <div class="p-6 text-sm text-stone-600">
            <ul class="list-disc pl-4 space-y-2">
                <li><strong>Anak</strong> (di atas 5 tahun)</li>
                <li><strong>Dewasa</strong> (di atas 17 tahun)</li>
            </ul>
        </div>
    </div>
</div>
...
(function(){
    ...
    function render() {
        ...
        tbody.innerHTML = page.map((p, i) => {
            let tambahanText = [];
            if (p.tambahanAnak > 0) tambahanText.push(`${p.tambahanAnak} Anak`);
            if (p.tambahanDewasa > 0) tambahanText.push(`${p.tambahanDewasa} Dewasa`);
            let finalTambahan = tambahanText.length > 0 ? tambahanText.join(', ') : '-';
            ...
            <tr class="border-b border-stone-100 hover:bg-amber-50/40 transition align-top cursor-pointer" ondblclick="showDetail(${AJUAN_DATA.indexOf(p)})">
                ...
                <td class="px-3 py-3 text-stone-700 text-xs">${p.tanggalDipesan || '-'}</td>
    ...
    window.showDetail = function(idx) {
        ...
        el.innerHTML = `
            <div class="grid grid-cols-2 gap-x-4 gap-y-2">
                ...
                <div><span class="text-stone-400 text-xs">Tanggal Dipesan</span><br><strong>${p.tanggalDipesan || '-'}</strong></div>
                ...
                <div class="col-span-2"><span class="text-stone-400 text-xs">Tambahan Orang</span><br><strong>${finalTambahan}</strong></div>
